...
public class CardboardGaze : MonoBehaviour {
  ...
  private void HandleTrigger() {
      ...
      if (!cardboardTrigger)
      {
            RaycastHit hit;
            Ray ray = new Ray(transform.position, transform.forward);
            Physics.Raycast(ray, out hit, cam.farClipPlane, mask);
            if (Cardboard.SDK.Triggered || hit.collider != null && hit.collider.isTrigger)
          {
              cardboardTrigger = true;
              //Start counter
              StartCoroutine(waitToChange(hit));
          }
            else if (!Cardboard.SDK.Triggered || hit.collider != null && !hit.collider.isTrigger)
            {
                // Trigger ended.
                if (pointer != null)
                {
    ...
    */
    ...
    IEnumerator waitToChange(RaycastHit hit)
    {
